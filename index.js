    ...
    export default function Home() {
      ...
      const addAddressTOWhitelist = async () => {
        try {
          ...
          const tx = await nftContract.addAddressToWhitelist(newaddress ? newaddress : "Add address ", name ? name : "Add name ",);
          setLoading(true);
     ...
     // issue degree 
      const issueDegree = async () => {
        try {
          ...
          const nftContract = new Contract(NFT_CONTRACT_ADDRESS, abi, signer);
          // call the issueDegree from the contract to issue a degree 
          const tx = await nftContract.issueDegree(issueto ? issueto : "studentname",);
          setLoading(true);
          // wait for the transaction to get mined
          await tx.wait();
          setLoading(false);
    ...
    // claim degree function
      const claimDegree = async () => {
        try {
          ...
          const nftContract = new Contract(NFT_CONTRACT_ADDRESS, abi, signer);
          // call the issueDegree from the contract to issue a degree 
          const tx = await nftContract.claimDegree(tokenuri ? tokenuri : "TokenURI",);
          setLoading(true);
          // wait for the transaction to get mined
          await tx.wait();
          setLoading(false);
    ...
    /*if (isOwner) {
      ...
      <div>
       ...
       <button className={styles.button} onClick={issueDegree}>
        ...
        return (
          ...
          <input id="claim" type="text" placeholder="claim Degree" onChange={onTokenuriChange}/>
        </form>
      ...
      return (
        <div>
          ...
          <footer className={styles.footer}>Made with &#10084; by Sharon & Nashons</footer>
        </div>
      );
    }